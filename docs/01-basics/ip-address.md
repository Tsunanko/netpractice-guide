...
## 🌍 アドレスの種類（3 つだけ覚える） {#private-vs-public}
...
!!! warning "⭐ プライベート IP は Internet に出られない"
    プライベート IP は **世界中で重複して使われている**（あなたの家も隣の家もマンションのお隣さんも `192.168.0.1` を持っている）。
    だから ISP のルータには「**プライベート IP 宛・プライベート IP 発のパケットは Internet に流さない**」という鉄のルールがある（RFC 1918）。

    - **家の中（同じルータ配下）** なら `192.168.x.x` で自由に通信できる
    - **Internet を跨ぐ通信**は、家庭用ルータが **NAT** という変換でパブリック IP に書き換えてから外に出す
    - NetPractice では NAT を扱わないので、**Internet が登場するレベル（L6, L8 など）の IP はすべてパブリック必須**

### 2. グローバル（パブリック）アドレス（世界の住所）

**Internet 上で一意** に決まる住所。Google の DNS サーバーの `8.8.8.8` が有名。
これを持っていないと、Internet のどこからも直接アクセスできない。

### NetPractice で「どっちを使うか」の早見表

| カテゴリ | 範囲 | NetPractice で登場するレベル |
|---|---|---|
| **プライベート** | `10.x.x.x` / `172.16〜31.x.x` / `192.168.x.x` | L7（閉じた LAN）など、**Internet が居ないレベル** |
| **パブリック** | 上記以外（`8.8.8.8`, `42.x.x.x`, `163.x.x.x` など） | **L6, L8（Internet あり）など** |

→ 各レベルの図に **Internet（🌍 アイコン）が居るか** で判断する。Internet が居れば全 IP がパブリック、閉じた LAN ならプライベート OK。
