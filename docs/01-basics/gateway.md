...
!!! info "💡 ここでつまずく人へ — ゲートウェイは「町外行きの最初の 1 歩」"
    パソコンが町の外にパケットを出すには、まず **町の中にある玄関ルータ** にバトンを渡します。
    玄関が **別の町** にあったら、そもそもバトンを渡せません（飛行機が無いので隣町の郵便局には行けない）。

    だから NetPractice でゲートウェイを書くときは
    **ホストの IP とマスクで AND した町名 = ゲートウェイの IP とマスクで AND した町名**
    が成り立つかを必ず確認してください。
...
!!! info "💡 ここでつまずく人へ — どうして `default` を「最後の手段」と呼ぶ？"
    `default` は「**他のどのルートにもマッチしなかった時の出口**」です。
    具体的なルート（例: `10.0.0.0/8 → R2`）を **先に** 書き、
    `default` を **一番下** に置くのが鉄則。順番を逆にすると、
    全部のパケットが `default` に吸い込まれて他のルートが無効化されます。

!!! info "💡 「ルート」と「ルーティングテーブル」って何？（次ページで詳しく）"
    ここで言う **「ルート」** は、ルータやホストが持っている
    **「宛先 → 次にどこへ送るか」という 1 行の指示** のこと。
    その指示を **何行も並べた表** が **ルーティングテーブル**。

    今は「上から順番に宛先とマッチするかを探し、見つかった行の『次の送り先』にパケットを渡す」
    という流れだけ知っていれば OK。
    詳しくは → [06. ルーティングテーブル](routing-table.md)
