...
!!! warning "ホスト部が全 0 / 全 1 の IP を使う → `invalid IP address` エラー"
    `192.168.1.0`（ネットワークアドレス）や `192.168.1.255`（ブロードキャスト）を **住人** に割り当てると、NetPractice では即座に **`invalid IP address`** という赤エラーで弾かれます。

    マスクが `/25` 以降になると **使えない番地が増える**ので注意：

    | マスク | このマスクで使えない値（第 4 オクテット） |
    |:-:|:---|
    | `/24` | `.0`, `.255` |
    | `/25` | `.0`, `.127`, `.128`, `.255` |
    | `/26` | `.0`, `.63`, `.64`, `.127`, `.128`, `.191`, `.192`, `.255` |

    各ブロックで「先頭」と「末尾」が NG。網羅的な早見表は [03. CIDR 早見表 §ブロードキャスト早見表](cidr.md#broadcast-quick-ref) 参照。
