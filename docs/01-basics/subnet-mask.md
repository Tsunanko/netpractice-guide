...
!!! warning "マスクのルール"
    ...
    **1 と 0 が混ざっている形は無効**（= NetPractice で弾かれる）。
    ...
    ✅ OK: `11111111.11111111.11111111.00000000` (= `/24`)
    ❌ NG: `11111111.11111111.11111111.00100000` (= `255.255.255.32`)
       ↑ 1 の後に 0 が挟まってから、また 1 が出ている
...
```
A の IP:   192.168.1.10    マスク /24
B の IP:   192.168.1.20    マスク /24

→ 町名は両方 192.168.1.0 → 同じ町 → 直接通話 OK
```

### 例 2: 違う町に住んでいる

```
A の IP:   192.168.1.10    マスク /24 → 町: 192.168.1.0
C の IP:   192.168.2.10    マスク /24 → 町: 192.168.2.0
...
```
192.168.1.0   ← 町全体を指す → 住民には割り当てられない
```
...
```
192.168.1.255 ← 町内全員 → 住民には割り当てられない
```

### 住人が使える範囲

```
192.168.1.0   ← NG（ネットワーク）
