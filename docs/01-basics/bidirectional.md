...
!!! info "💡 ここでつまずく人へ — 「ping が返って来ない」って何が起きてる？"
    ping は **「行きのパケットを送って、戻りのパケットが返って来るまでの時間」** を測ります。
    だから **行きが届いても帰りが消えると「unreachable」と表示** されます。

    NetPractice で「行きの絵だけ完成させて満足する」とほぼ確実にここで詰まるので、
    **必ず「相手側から自分への route」も用意できているか** を確認してください。
...
| gate | `163.172.250.12` （R のインターフェイス） |

意味: 「`192.168.1.0/24` 宛てのパケットは R に投げ返してね」
