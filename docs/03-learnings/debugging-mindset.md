...
<div class="step-flow">
  <div class="step"><span class="step-num">1</span><b>固定</b><br>Dr1 gate<br>= <code>32.30.74.78</code></div>
  <div class="step"><span class="step-num">2</span>だから<br>R23 の IP は<br><code>32.30.74.78</code></div>
  <div class="step"><span class="step-num">3</span>だから<br>D の町は<br><code>32.30.64.0/18</code></div>
  <div class="step"><span class="step-num">4</span>だから<br>D1 の IP は<br><code>32.30.64.x</code></div>
</div>

**「自由に設計して」と言われたら迷うが、固定値があることで「こうするしかない」が見えてくる**。
...
<div class="step-flow">
  <div class="step"><span class="step-num">1</span><b>事実</b><br>この API<br>だけが落ちる</div>
  <div class="step"><span class="step-num">2</span>だから<br>共通処理・DB<br>の問題ではない</div>
  <div class="step"><span class="step-num">3</span>だから<br>このエンドポイント<br>特有のコード<br>/データの問題</div>
  <div class="step"><span class="step-num">4</span>だから<br>最近の変更<br>or 特定レコード<br>を調べる</div>
  <div class="step"><span class="step-num">5</span>具体策<br><code>git log</code> で<br>直近 7 日の<br>変更を見る</div>
</div>
...
- 自前実装は時間的に**無理** → Django の標準認証を使う
- MFA は必須要件に入っていない → **MVP では外す**
- OAuth プロバイダは Google のみ（社内で承認済み）

**結論**: Django-allauth + Google OAuth で 1 週間で出す。
...
**熟練の回答** は、まず**面接官に制約を聞く**:

- 想定ユーザー数は？（100 万？ 10 億？）
- 書き込みと読み込みの比率は？（1:100？ 1:1000？）
- レイテンシ要件は？（1 ms？ 100 ms？）
- 可用性要件は？（99%？ 99.999%？）
- 地理的分散は必要？
- 予算制約は？

ここまで聞いて、**制約が決まってから設計を描き始める**。

**制約が全て決まれば、設計は「こうするしかない」になる**。自由度が多いうちは議論も実装も進まない。
...
| 変えられないもの | 変えられるもの |
|:---|:---|
| 既存の DB スキーマ（本番データあり） | 内部の実装 |
| 外部 API の仕様 | テストの書き方 |
| 予算と納期 | リリース戦略 |
| チームメンバーのスキルセット | — |
| 会社のコーディング規約 | — |

**変えられないもの** から先に考えると、**変えられるもの** の自由度の中で最適な実装が見えてくる。
