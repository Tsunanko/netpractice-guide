...
!!! tip "読み手ファースト"
    ...
    **専門用語は使う前に必ず平易な言葉で説明** し、**表とたとえ話** を多用します。
...
!!! info "ガイドはあくまで考え方の解説"
    提出 JSON の数値は **intra login ごとにランダム生成** されるため、
    このサイトに載っている数値をそのままコピーしても **別アカウントの正解にはなりません**。
    **「どう考えて解くか」** を学ぶのがこのサイトの目的です。

!!! warning "利用について（免責）"
    - このガイドは **学習ノート** として公開しています。無断転載は避けてください
    - 記載の数値は **例** です。**そのまま提出しても動きません**
    - 評価（ディフェンス）では **自分の言葉で説明** できる必要があります。
      コピペ丸暗記は見抜かれるので、必ず理解してから臨んでください
    - 42 の academic integrity（学術的誠実さ）を尊重してください
