...
L8 はこのガイド全体の **山場**。ネットワーク設計の核となる **「集約 (route summarization)」** という発想が初めて要求されるレベル。先に答えだけ並べると：

1. **複数の固定値の連鎖から逆算する** — 編集可能な値より、**固定値（薄ピンク）が解の手がかり**。R2 の routes 右側 (`.62`) → R13 の IP は `.62` と一致 → R13 のマスクは Internet の routes の `/26` と一致、というように **制約が連鎖** する。固定値を並べて「これと一致しなきゃダメな値はどれか」を 1 つずつ確定させていく
2. **C と D を「隣接 /28 ブロック」に並べる** ⭐ — D の街が `.0/28` (`.0〜.15`) なら、C を **すぐ隣の** `.16/28` (`.16〜.31`) に置く。これが「集約」を可能にする鍵。隣接していないと `/27` で 1 本にまとめられない
3. **`/27` 集約 — 隣接した 2 つの `/28` を 1 行のルートに** ⭐ — `.0/28` と `.16/28` を合わせて `.0/27` (`.0〜.31`) と書ける。R1 の routes が **1 本だけしか書けない制約** を解くために、この集約が必須
4. **直結ルートは書かない** — R1 の routes に書くのは「**自分が直結していない宛先**」だけ。C と D の街への routes が `/27` 集約で 1 本、Internet 行きが `default` で 1 本、合計 **2 本** で済む（→ [next hop 三原則](../01-basics/routing-table.md#next-hop)）
5. **Internet 接続あり → 全 IP パブリック必須** — このレベルの `149.x.x.x` `163.x.x.x` `7.x.x.x` は意図的にすべてパブリック IP。プライベート IP（`192.168.x.x` など）は使えない（→ [プライベート IP の制約](../01-basics/ip-address.md#private-vs-public)）

詳細は下の「📷 実際のスクリーンショットで解く（具体例）」と「🧠 考え方の核心」で順番に追っていきます。
