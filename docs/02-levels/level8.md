...
ブロック `161.138.113.0/26` の住人は `.1〜.62`。そこに 2 つの IF を配置する。

| 項目 | 値 | 備考 |
|:---|:---|:---|
| ブロック | `161.138.113.0/26` | 住人 `.1〜.62` |
| R13 IP | `161.138.113.62` | R2r1 gate の固定値がこの IP を指している |
| R21 IP | `161.138.113.1` | 住人範囲の空いている値 |
| マスク | `255.255.255.192`（`/26`） | R13 と R21 で同じ |

### Step 3: D と C を隣接ブロックに配置 ⭐

問題: **R1 で編集できるルートは 1 本のみ**。でも C と D の **両方への戻り道** が必要。

解決策: C と D を **隣り合った `/28` ブロック** に配置して、**`/27` でまとめて 1 本のルート** で包む。

| ブロック | 用途 | 範囲（使える IP） | R1 から見た役割 |
|:---|:---|:---|:---|
| `7.9.10.0/28` | D の町 | `.1〜.14` | ↓ |
| `7.9.10.16/28` | C の町 | `.17〜.30` | ↓ |
| **`7.9.10.0/27`** | **集約して 1 本のルート** | `.1〜.30`（両方含む） | ✅ これ 1 本で両 LAN に届く |

各 IF の割り当て:

- D 側: D1 = `7.9.10.11`（固定）、D の町 = `.0/28`、R23 = `7.9.10.1`
- C 側: C の町 = `.16/28`、R22 = `7.9.10.17`、C1 = `7.9.10.18`
